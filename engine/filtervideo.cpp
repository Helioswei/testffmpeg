...
static int open_input_file(const char * filename){
	...
	for(int i = 0; i < ifmt_ctx -> nb_streams; i++){
		...
		if (!dec){
			av_log(NULL, AV_LOG_ERROR, "Failed to find decoder fro stream #%u\n",i);
...
static int open_output_file(const char * filename){
	...
	for(int i = 0; i < ifmt_ctx -> nb_streams; i++){
		...
		if (dec_ctx -> codec_type == AVMEDIA_TYPE_VIDEO || dec_ctx -> codec_type == AVMEDIA_TYPE_AUDIO){
			...
			if (!encoder){
				av_log(NULL, AV_LOG_ERROR, "Necesaary encoder not found\n");
				return AVERROR_UNKNOWN;
			}
			...
			/*In this example , we transcode to same properties(picture size, sample rate etc.).These properties can be
			 * changed for ouput streams easily using filters*/
...
static int init_filter(FilteringContext * fctx, AVCodecContext *dec_ctx, AVCodecContext *enc_ctx, const char *filter_spec)
{
	...
	if (dec_ctx -> codec_type == AVMEDIA_TYPE_VIDEO){
		...
		if (ret < 0){
			av_log(NULL, AV_LOG_ERROR, "Canot set output sample format \n");
			goto end;
		}
		...
		//ret = av_opt_set_bin(buffersrc_ctx, "sample_rates", (uint8_t*)&enc_ctx ->sample_rate,sizeof(enc_ctx -> sample_rate),AV_OPT_SEARCH_CHILDREN);
		//if (ret < 0){
...
static int init_filters(void){
	...
	int ret;
	filter_ctx =(FilteringContext*) av_mallocz_array(ifmt_ctx ->nb_streams, sizeof(*filter_ctx));
	if (!filter_ctx){
		return AVERROR(ENOMEM);
	}
	...
	for (int i = 0; i < ifmt_ctx -> nb_streams; i++){
		...
		if (ifmt_ctx -> streams[i] -> codecpar -> codec_type == AVMEDIA_TYPE_VIDEO)
			...
			//filter_spec = "drawtext=fontfile=/usr/share/fonts/fonts\\\\/simsun.ttc:fontsize=100:text=hello world:x=100:y=100:fontcolor=yellow@0.5:box=1:boxcolor=red@0.2";//passthrough (dummy)filter for video
			//filter_spec = "movie=/root/source_img/girl.png[wm];[in][wm]overlay=5:5[out]";
			/*[wm]是作为movie过滤器的输出名字，[in][wm]作为overlay过滤器的输入名字*/
		else
