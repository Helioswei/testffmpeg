...
int main(int argc, char* argv[]){
	...
	int output_frame_size = enc_ctx -> frame_size;
	while(true){
		...
		if (packet.stream_index == audioIndex){
			...
			av_frame_free(&frame);
			//将重采样后的数据放入队列中
			ret = av_audio_fifo_realloc(fifo, av_audio_fifo_size(fifo) + len);
			if (ret < 0){
				av_log(NULL, AV_LOG_ERROR, "Could not reallocate FIFO\n");
				break;
			}
			if (av_audio_fifo_write(fifo,(void **)areframe -> data,len) < len){
				av_log(NULL, AV_LOG_ERROR, "Could not write data to FIFO\n");
				break;
			}
			//判断当前队列中的数据大小与output_frame_size的大小，如果小于则在读取
			if (av_audio_fifo_size(fifo) < output_frame_size){
				av_log(NULL, AV_LOG_DEBUG, "continue to read packet");
			...
			while( av_audio_fifo_size(fifo) >= output_frame_size){
				...
				if (init_output_frame(&areframe, enc_ctx, frame_size) < 0){
					av_frame_free(&areframe);
					break;	
				}
				if (av_audio_fifo_read(fifo,(void **)areframe -> data,frame_size) < frame_size){
					av_log(NULL, AV_LOG_ERROR, "av_audio_fifo_read failed\n");
					av_frame_free(&areframe);
					break;
				}
				areframe -> pts = mypts;
