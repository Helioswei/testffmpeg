	...
	while (true){
		...
		if (type == AVMEDIA_TYPE_VIDEO){
			...
			if (got_frame){
				...
				tmppkt.data = NULL;
				tmppkt.size = 0;
				//必须要加这个，不然avcodec_encode_video2会崩溃
				av_init_packet(&tmppkt);
				ret = avcodec_encode_video2(stream_ctx[stream_index].enc_ctx, &tmppkt, frame, &got_output);
				if (ret < 0){
					av_frame_free(&frame);
				...
				if (got_output){
					...
					/*prepare packet for muxing,这个也是必须加的*/
			...
			av_log(NULL, AV_LOG_ERROR, "lalaalaalla");
		}
